...
datafile = os.path.join(corpus, "volunteer_model_formatted.txt")
...
def filterPair(p):
    ...
    return len(p) > 1 and len(p[0].split(' ')) < MAX_LENGTH and len(p[0].split(' ')) > 1 and len(p[1].split(' ')) < MAX_LENGTH and len(p[1].split(' ')) > 1
...
def loadPrepareData(corpus, corpus_name, datafile, save_dir):
    ...
    for pair in pairs:
        pair[0] = clearDuplicate(pair[0])
        pair[1] = clearDuplicate(pair[1])
...
def readFormattedFile(file):
    with open(file, 'r') as datafile:
        lines = datafile.readlines()
    return lines

formattedLines = readFormattedFile(os.path.join(corpus, "volunteer_model_formatted.txt"))
